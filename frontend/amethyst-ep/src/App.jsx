...
export default function App() {
  ...
  if (!authReady) return <main className="auth-page"><div className="page-image"><AmethystPlus value={view} onChange={setView} /></div><p role="status">Opening workspace…</p></main>
  if (!user) return <AuthPanel view={view} onChangeView={setView} onAuthenticated={user => { setUser(user); setError(''); void refresh() }} onBack={() => setView(null)} />
  ...
  return <div className="workspace">
    ...
    <div className="workspace-main"><header className="workspace-header"><span>{views[view]} <span className="header-divider">/</span> Overview</span><div className="account-menu"><span className="avatar">{user.name[0]}</span><span>{user.name}<small>{canEdit ? 'Administrator' : 'Reader'}</small></span><button className="text-button" onClick={logout}>Sign out</button></div></header>
      <main className="workspace-content">
        <div className="page-image workspace-image"><AmethystPlus value={view} onChange={nextView => { setView(nextView); setPanel(null) }} /></div>
        ...
        {loading ? <div className="loading-card" role="status">Loading workspace…</div> : error ? <div className="empty-state"><h2>Unable to load the workspace.</h2><p>Try refreshing to reconnect.</p></div> : view === 'business' ? <TicketDashboard onSave={saveTicket} people={members.map(m => m.name)} tickets={tickets} canEdit={canEdit} onOpen={setPanel} onCreate={() => setPanel('new')} /> : view === 'care' ? <RoadmapView people={members.map(m => m.name)} items={roadmap} canEdit={canEdit} onSave={saveRoadmap} onExecute={executeRoadmap} onOpenTicket={id => setPanel(tickets.find(t => t.id === id))} /> : <TeamView tickets={tickets} members={members} canEdit={canEdit} onSaveMember={saveMember} onAssign={owner => setPanel({ newTicket: true, owner })} onOpen={setPanel} />}
      </main>
    </div>
