...
export default function TicketPanel({ ticket, canEdit, onClose, onSave, people, initialOwner = '', initialDraft, importLabel, onCancelImport }) {
  const dialog = useRef(null)
  const [editing, setEditing] = useState(!ticket)
  const [draft, setDraft] = useState(ticket || { ...empty, owner: initialOwner, ...initialDraft })
  ...
  return <dialog ref={dialog} className="ticket-dialog" onCancel={(event) => { event.preventDefault(); if (!saving) onClose() }} aria-labelledby="ticket-heading">
    <div className="panel-heading"><div><p className="eyebrow">{ticket ? `ACTION ITEM · AP-${String(ticket.id).padStart(3, '0')}` : 'NEW ACTION ITEM'}</p><h2 id="ticket-heading">{editing ? ticket ? 'Edit ticket' : importLabel ? 'Review imported ticket' : 'Create a ticket' : 'Ticket details'}</h2></div><button className="icon-button" onClick={onClose} disabled={saving} aria-label="Close ticket">✕</button></div>
    {editing && canEdit ? <form onSubmit={submit}>
      ...
      <div className="panel-actions">{onCancelImport && <button type="button" className="text-button" disabled={saving} onClick={onCancelImport}>Discard remaining</button>}<button type="button" className="secondary-button" onClick={onClose} disabled={saving}>{importLabel ? 'Skip item' : 'Cancel'}</button><button className="primary-button" disabled={saving}>{saving ? 'Saving…' : ticket ? 'Save changes' : importLabel ? 'Approve & add ticket' : 'Create ticket'}</button></div>
